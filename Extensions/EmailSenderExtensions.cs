    ...
    public static class EmailSenderExtensions
    {
        public static Task SendEmailConfirmationAsync(this IEmailSender emailSender, string email, string link)
        {
            return emailSender.SendEmailAsync(email, "Confirm your email",
                $"Please confirm your {emailSender.SiteName} account by <a href='{HtmlEncoder.Default.Encode(link)}'>clicking this link</a>.");
        }
        ...
        public static Task SendNewEmailChangeAsync(this IEmailSender emailSender, string email)
        {
            return emailSender.SendEmailAsync(email, "Change your email",
                $"Someone requested to change your {emailSender.SiteName} email address. If you confirm the change, this (<strong>{email}</strong>) will be your new address.");
        }

        public static Task SendOldEmailChangeAsync(this IEmailSender emailSender, string email, string newEmail, string link)
        {
            return emailSender.SendEmailAsync(email, "Change your email",
                $"Someone requested to change your {emailSender.SiteName} email address (the new address will be <strong>{newEmail}</strong>).<br/>" +
                $"Ignore this email if you did not request it!<br/>" +
                $"<br/>" +
                $"To confirm the change to your email <a href='{HtmlEncoder.Default.Encode(link)}'>click this link</a>.");
        }
        ...
        public static Task SendEmailFiatDepositCreatedAsync(this IEmailSender emailSender, string email, string asset, string amount, string depositCode, xchwallet.BankAccount account)
        {
            return emailSender.SendEmailAsync(email, "Deposit Create",
                $"A new fiat deposit created, {amount} {asset}<br/>" +
                $"<br/>" +
                $"Deposit Code: {depositCode}<br/>" +
                $"Bank Name: {account.BankName}<br/>" +
                $"Bank Address: {account.BankAddress}<br/>" +
                $"Account Name: {account.BankName}<br/>" +
        ...
        {
            return emailSender.SendEmailAsync(email, "Deposit Confirmed",
                $"A fiat deposit has confirmed, {amount} {asset}<br/>" +
                $"<br/>" +
                $"Deposit Code: {depositCode}<br/>");
        }
        ...
        {
            return emailSender.SendEmailAsync(email, "Confirm your account creation",
                $"Someone has requested to create a {emailSender.SiteName} account using this email address.<br/>" +
                $"Only click the link if it was you who created this request.<br/>" +
                $"<br/>" +
                $"Confirm your account by <a href='{HtmlEncoder.Default.Encode(link)}'>clicking this link</a> (expires in {expiryMins} minutes).");
        }
        ...
        public static Task SendEmailApiKeyCreationRequest(this IEmailSender emailSender, string email, int expiryMins, string link)
        {
            return emailSender.SendEmailAsync(email, "Confirm API KEY creation",
                $"Someone has requested to create an {emailSender.SiteName} API KEY for your account.<br/>" +
                $"Only click the link if it was you who created this request.<br/>" +
                $"<br/>" +
                $"Confirm your new API KEY creation by <a href='{HtmlEncoder.Default.Encode(link)}'>clicking this link</a> (expires in {expiryMins} minutes).");
        }
